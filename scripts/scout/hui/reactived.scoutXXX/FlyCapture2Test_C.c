   ...
   */ 
   // Retrieve the image
     ...
     if ( error == FC2_ERROR_OK )  
     {
        ...
        //error = fc2ConvertImageTo(FC2_PIXEL_FORMAT_BGR, &rawImage, &convertedImage);   // BGR is wrong for our purpose
        error = fc2ConvertImageTo(FC2_PIXEL_FORMAT_RGB, &rawImage, &convertedImage);    // now it works !!!!!!!!!!!!!!!!!!!!!!!
        if ( error != FC2_ERROR_OK )
        {
        ...
        if(numImagesToGrab == 0){
		error = fc2SaveImage( &rawImage, "fc2TestImageRaw.png", FC2_PNG );
		error = fc2SaveImage( &convertedImage, "fc2TestImageConverted.png", FC2_PNG );}
        else {

                error = fc2SaveImage( &rawImage, "fc2TestImageRaw2.png", FC2_PNG );
                error = fc2SaveImage( &convertedImage, "fc2TestImageConverted2.png", FC2_PNG );}
         */

         ptr = convertedImage.pData;
         for(k=0;k<convertedImage.rows*convertedImage.cols*3;k++){
            vidbuf[k] = *ptr++;
	 }
...
void *videoOffsetFlyCap(int blockid){
