...
/* say something */
/* Modified 03/05 by mark Obsniuk */
void say(char *path)
{
	char buf[1024];
	sprintf(buf, "./sounds/aplay %s &", path);
	system(buf);	
}
...
{    printf("Here in TimeSlice ========== TimeSlice ==================== \n");
...
// added by HuiWang. modified from above
void snapPictureFlyCap(float *x, float *y, float *theta)
{
# ifdef DOVIDEO
	int dcmd;
        printf("---------------== here in 'snapPictureFlyCap' ==-------------- \n");
...
# endif
	...
	*theta = robot.theta;
	//videoTakePic(mfd);
        ...
        doPicture(0); // defined in FlyCaptureTest2_C.c
# endif
}
